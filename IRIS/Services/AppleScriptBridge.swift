...
class AppleScriptBridge {
    ...
    ///   - amount: Number of lines/units
    func scroll(direction: String, amount: Int = 5) async throws {
        var scrollY = 0
        var scrollX = 0
        ...
        case "right":
            scrollX = -amount
        default:
            scrollY = -amount // Default down
        }
        
        let event = CGEvent(scrollWheelEvent2Source: nil, units: .line, wheelCount: 2, wheel1: Int32(scrollY), wheel2: Int32(scrollX), wheel3: 0)
        event?.post(tap: .cghidEventTap)
    }
