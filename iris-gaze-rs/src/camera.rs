...
use opencv::{
    ...
    imgproc,
    prelude::*,
    videoio::{self, VideoCapture, CAP_ANY, CAP_AVFOUNDATION},
};
...
impl Camera {
    ...
    pub fn new(camera_index: i32, width: i32, height: i32, fps: i32) -> Result<Self, CameraError> {
        log::info!(
            "Initializing OpenCV camera index {}: {}x{} @ {}fps",
            camera_index,
            width,
            height,
            fps
        ...
        // Only try the requested camera index — probing multiple indices/backends
        // can crash the macOS camera subsystem and makes startup slow.
        let mut capture = VideoCapture::new(camera_index, CAP_AVFOUNDATION)?;
        if !capture.is_opened()? {
            // Single fallback: try CAP_ANY on same index
            if let Ok(mut f) = std::fs::OpenOptions::new()
                .create(true)
                .append(true)
                .open("/tmp/iris_rust.log")
            {
                ...
                let _ = writeln!(
                    f,
                    "⚠️ CAP_AVFOUNDATION failed for index {}, trying CAP_ANY",
                    camera_index
                );
            }
            capture = VideoCapture::new(camera_index, CAP_ANY)?;
            if !capture.is_opened()? {
        ...
        {
            use std::io::Write;
            let _ = writeln!(f, "✅ Camera {} opened successfully", camera_index);
        }
        ...
        {
            use std::io::Write;
            let _ = writeln!(f, "✅ Camera ready for capture");
        }
