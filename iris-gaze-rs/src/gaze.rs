...
impl GazeEstimator {
    ...
    pub fn new(screen_width: i32, screen_height: i32, _ema_alpha: f32, deadzone: f32) -> Self {
        ...
        let min_cutoff = 1.8; // Balanced smoothing
        let beta = 0.7; // Good speed adaptation
    ...
    pub fn estimate(&mut self, landmarks: &FaceLandmarks) -> Option<(f64, f64)> {
        ...
        let mut h_norm = (ema_nose_x - self.nose_x_min) / (self.nose_x_max - self.nose_x_min);
        let mut v_norm = (ema_nose_y - self.nose_y_min) / (self.nose_y_max - self.nose_y_min);

        // Apply deadzone to normalized coordinates (widen at higher gain for stability)
        let deadzone = if gain_avg >= 3.0 {
            0.16
        } else if gain_avg >= 2.5 {
            0.12
        } else {
            self.deadzone as f64
        };
        ...
        // Extra smoothing for amplified reach to prevent jitter
