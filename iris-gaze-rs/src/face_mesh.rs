...
pub struct FaceMeshDetector {
    ...
    /// Smoothing factor (EMA alpha) — lower = smoother landmarks
    alpha: f32,
    /// Frame counter
    frame_count: u32,
    /// Last detected face (raw from detector)
    last_face: Option<Rect>,
    /// Smoothed face rect (EMA on bounding box to avoid crop jumps)
    smoothed_face: Option<(f32, f32, f32, f32)>,
    /// Whether initialized
    initialized: bool,
    /// Image dimensions
    img_width: f32,
...
impl FaceMeshDetector {
    pub fn new() -> Result<Self, FaceMeshError> {
        ...
        let haar_paths = [
            ...
            "/Users/livio/Documents/iris/gaze_env/lib/python3.11/site-packages/cv2/data/haarcascade_frontalface_default.xml",
        ];
        ...
        Ok(Self {
            ...
            alpha: 0.5, // EMA smoothing factor — increased from 0.2 for more responsiveness
            frame_count: 0,
            last_face: None,
            smoothed_face: None,
            initialized: false,
    ...
    /// Smooth the face bounding box to prevent crop region jumps between frames
    fn smooth_face_rect(&mut self, face: &Rect) -> Rect {
        let face_alpha = 0.6_f32; // Increased from 0.3 for more responsiveness
        let (x, y, w, h) = (
            face.x as f32,
            face.y as f32,
            face.width as f32,
        ...
        if let Some((sx, sy, sw, sh)) = self.smoothed_face {
            let nx = sx + (x - sx) * face_alpha;
            let ny = sy + (y - sy) * face_alpha;
            let nw = sw + (w - sw) * face_alpha;
            let nh = sh + (h - sh) * face_alpha;
            self.smoothed_face = Some((nx, ny, nw, nh));
            Rect::new(nx as i32, ny as i32, nw as i32, nh as i32)
        } else {
            self.smoothed_face = Some((x, y, w, h));
            *face
        }
    }
    ...
    fn detect_landmarks_onnx(
        ...
        if log_count <= 5 && landmarks_data.len() >= 33 {
            ...
            let norm_nx =
                1.0 - (crop_x1 as f32 + (nx * crop_width as f32 / 192.0)) / frame.width as f32;
            let norm_ny =
                (crop_y1 as f32 + (ny * crop_height as f32 / 192.0)) / frame.height as f32;
            let norm_fx =
                1.0 - (crop_x1 as f32 + (fx * crop_width as f32 / 192.0)) / frame.width as f32;
            let norm_fy =
    ...
    pub fn detect(&mut self, frame: &Frame) -> Result<Option<FaceLandmarks>, FaceMeshError> {
        ...
        let raw_face = faces.get(0)?;
        // Smooth the face bounding box to prevent crop region jumps
        let face = self.smooth_face_rect(&raw_face);
