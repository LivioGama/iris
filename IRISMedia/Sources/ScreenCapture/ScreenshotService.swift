...
public class ScreenshotService {
    ...
    /// Draws a gaze point (orange dot) on an image
    /// - Parameters:
    ///   - image: The NSImage to draw on
    ///   - point: The point to draw at (in macOS coordinates - bottom-left origin)
    ///   - radius: The radius of the dot (default: 20)
    /// - Returns: New NSImage with gaze point drawn
    public func drawGazePoint(on image: NSImage, at point: CGPoint, radius: CGFloat = 20) -> NSImage {
        let imageSize = image.size
        let newImage = NSImage(size: imageSize)
        ...
        // Convert point from screen to image space
