...
- `ServiceProtocols.swift` - Defines all service interfaces:
  ...
  - `IntentResolutionService` - Intent parsing
  - `ContextualAnalysisServiceProtocol` - Contextual element analysis
...
#### Analysis
- `ContextualAnalysisService.swift` - Contextual element analysis
  - Gaze point analysis
...
### 1. Blink Detection → Gemini Analysis Flow

```
User Blinks
     ↓
GazeEstimator detects blink
     ↓
ScreenCaptureService captures screenshot
     ↓
GeminiClient sends image + context to Gemini API
     ↓
ConversationManager adds to history
     ↓
Gemini analyzes and responds
     ↓
GeminiResponseOverlay displays result
```
...
1. **Initialization**: DependencyContainer creates all services
2. **Setup**: Services configure and validate dependencies
3. **Ready**: Main loop starts, Python subprocess launches
4. **Running**: Event processing, gaze tracking, intent detection
